...
cattleinv_data_yearly= as.data.table(cattleinv_data_yearly)

cattleinv_data_2021 =  read_csv("data/2021_beef_cattle_inv_state.csv")
cattleinv_data_2021= as.data.table(cattleinv_data_2021)
```
...
```{r}
table_data = cattleinv_data_yearly
table_data[, Value:=Value/1000]
table_data = setorderv(table_data, cols="Year",  order=1L, na.last=TRUE)
table_data[, Value:=round(Value, digits = 0)]

knitr::kable(
  table_data[82:103],
  col.names = c('Year', 'Production'),
  align = "lccrr",
  caption = "U.S. Beef Cattle Inventory,January 1-1000 heads"
)
...
ggplot(dt, aes(x = Year, y = Inventory)) + 
  labs(title = "U.S. Beef Cattle Inventory,January 1-1000 heads") +
...
table_data = cattleinv_data_yearly
table_data = setorderv(table_data, cols="Year",  order=1L, na.last=TRUE)
table_data =table_data %>% mutate(pct_change = (Value/lag(Value) - 1) * 100)
table_data = as.data.frame(table_data)
table_data = table_data[83:103,]

#head(dt, 5)

ggplot(table_data, aes(x = Year, y = `pct_change`)) + 
  labs(title = "U.S. Beef Cattle Inventory,January 1-%-change") +
  geom_line(color="orange") +
  theme_minimal()
```

```{r}
table_data = cattleinv_data_2021
setnames(cattleinv_data_2021,  "State","state")
table_data[, Value:=round(Value, digits = 0)]
table_data[, Value:=Value/1000]
table_data = setorderv(table_data, cols="Value",  order=-1L, na.last=TRUE)

knitr::kable(
  table_data[1:10],
  col.names = c('Year', '1000 Heads'),
  align = "lccrr",
  caption = "U.S. Beef Cattle Inventory, by State, 2021-1000 heads"
)
...
plot = 
  ...
  labs(title = "U.S. Beef Cattle Inventory, by State,2021-1000 heads") +
...
```{r}
table_data =  beef_prod_data[, -c(3:10), with=FALSE]
table_data[, production:=round(production, digits = 0)]
table_data = setorderv(table_data, cols="year",  order=1L, na.last=TRUE)
...
knitr::kable(
  ...
  col.names = c('Year', 'Production'),
  align = "lccrr",
  caption = "U.S. Beef Production - 1000 lbs"
)
...
```{r}
table_data =  beef_prod_data[, -c(3:10), with=FALSE]
table_data =table_data %>% mutate(pct_change = (production/lag(production) - 1) * 100)
table_data = as.data.frame(table_data)
table_data = table_data[31:52,]


#head(dt, 5)

ggplot(table_data, aes(x = year, y = `pct_change`)) + 
  labs(title = "U.S. Beef Production-%-change") +
  geom_line(color="orange") +
  theme_minimal()
```
...
table_data[, `farm-retail`:=round(`farm-retail`, digits = 0)]
table_data[, `farm-wholesale`:=round(`farm-wholesale`, digits = 0)]
table_data[, `wholesale-retail`:=round(`wholesale-retail`, digits = 0)]

knitr::kable(
  table_data[613:624],
  col.names = c('Date', 'Farm-Retail', 'Farm-Wholesale', 'Wholesale-Retail'),
  align = "lccrr",
  caption = "U.S. Beef Price Spreads- Cent per pound"
)
