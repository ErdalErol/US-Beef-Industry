...
knitr::opts_chunk$set(echo = TRUE, error = TRUE, warning= FALSE, message = FALSE, global.true=TRUE,
                      comment = '', fig.width = 15, fig.height = 10, out.width='750px')
```
...
# Packages Required
This project aims to provide a snapshot of U.S. beef industry with R data wrangling, visualization and mapping functions and techniques. 
...
![https://unsplash.com/photos/J_460ZyUfi8?utm_source=twitter&utm_medium=referral&utm_content=creditShareLink ](images/daiga-ellaby-J_460ZyUfi8-unsplash.jpg){#id .class width=750 height=250px}
...
knitr::include_graphics(rep("images/david-dolenc-1AKuVmQxUkU-unsplash.jpg"))
```
...
g1 =ggplot(dt, aes(x = Year, y = Inventory)) + 
...
g1 = ggplot(table_data, aes(x = Year, y = `pct_change`)) + 
...
# Set label font size
g1$layers[[2]]$aes_params$size <- 1.5
...
```{r}
dt = 
  beef_prod_data[, c(1:2), with=FALSE] %>% 
  as.data.frame(dt)  %>%
  select(year, production) %>%
  gather(key = "variable", value = "value", -year)

g1 = ggplot(dt, aes(x = year, y = value)) + 
  labs(title = "U.S. Beef Production-Million pounds") +
  geom_line(aes(color = variable), size = 1) +
  scale_color_manual(values = c("orange")) +
  theme_bw()
...
g1 = ggplot(dt, aes(x = year, y = Change)) + 
...
g1 = ggplot(data1, aes(x = "" , y = export_share, fill=fct_inorder(country))) +
...
data2 = as.data.frame(data2)
g1 = ggplot(data1, aes(x = "" , y = import_share, fill=fct_inorder(country))) +
...
g1= ggplot(dt, aes(x = date, y = value)) + 
...
```{r}
dt = 
  beef_prod_data[, c(1,9), with=FALSE] %>% 
  as.data.frame(dt)  %>%
  select(year, population) %>%
  gather(key = "variable", value = "value", -year)

g1 = ggplot(dt, aes(x = year, y = value)) + 
  labs(title = "U.S. Population-Thousand") +
  geom_line(aes(color = variable), size = 1) +
  scale_color_manual(values = c("orange")) +
  theme_bw()
...
g1 = ggplot(dt, aes(x = year, y = value)) + 
  ...
  geom_line(aes(color = `per capita consumption`), size = 2) +
  scale_color_manual(values = c("orange", "lightblue", "lightgreen")) +
  theme_bw()
