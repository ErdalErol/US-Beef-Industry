...
table_data %>%
  ...
  ), colnames = c('Year', '# of Heads-1000', 'Change'))%>%
  formatCurrency('Value',currency = "", interval = 3, mark = ",")%>%
  formatPercentage('pct_change', 2)
...
table_data = as.data.table(cattleinv_data_2021) %>% arrange(-Value)
...
table_data %>% as.data.table(cattleinv_data_2021)%>%
  ...
  ), colnames = c('State', '# of Heads-1000'))%>%
  formatCurrency('Value',currency = "", interval = 3, mark = ",")
